...
*   **严格执行**: **[NEW]** 强化了错误处理机制。如果架构师阶段执行失败（如 JSON 解析错误或字段缺失），系统将直接记录详细的错误日志（包含堆栈信息）并中断后续操作，确保不会在错误的蓝图上进行编辑。
*   **输出**: 包含 `expertTitle` 和 `strategicDirection` 的 JSON 蓝图。
...
### 4. 灵活的模型配置
*   **Gemini & OpenAI**: 支持最新的 Gemini 3 系列模型及所有 OpenAI 兼容接口。
*   **自定义模型名**: 现在支持手动输入任何模型名称，方便测试最新的预览版模型。
